...
export default {
    ...
    methods: {
        selectMode(mode) {
            this.gameMode = mode;
            this.currentScreen = mode === 'individual' ? 'individualForm' : 'teamForm';
        },
        showTeamMembersForm() {
        ...
        async startQuiz() {
            ...
            try {
                ...
                // If less questions returned than needed, add some general questions
                if (this.questions.length < this.totalQuestions) {
                    console.log(`Only ${this.questions.length} questions available. Adding general questions.`);
                    
                    // Try to fetch more general questions
                    const generalResponse = await fetch(`/api/questions?limit=${this.totalQuestions - this.questions.length}`);
                    const generalQuestions = await generalResponse.json();
                    
                    // Filter out duplicates
                    const existingIds = this.questions.map(q => q.id);
                    const additionalQuestions = generalQuestions.filter(q => !existingIds.includes(q.id));
                    
                    // Add additional questions
                    this.questions = [...this.questions, ...additionalQuestions.slice(0, this.totalQuestions - this.questions.length)];
                }
